...
de urgência) -> treino do modelo.

Todos os artefatos (dataset processado e modelo .joblib) são escritos em
`/opt/airflow/project/{data,models}`, que são montados no host a partir de
...
with DAG(
    ...
    description="Ingestão, pré-processamento e treino do classificador de triagem.",
    schedule=None,
